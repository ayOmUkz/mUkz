...
The skill writes no credentials, reads no environment variables, and makes no network calls beyond:

- `yfinance` → Yahoo Finance (`query1.finance.yahoo.com`, `query2.finance.yahoo.com`) — primary.
- `stooq.com` — automatic fallback if Yahoo returns empty data (no key, no signup). Tickers are mapped: US equities/ETFs append `.us` (e.g. `AAPL` → `aapl.us`); crypto pairs strip the dash (e.g. `BTC-USD` → `btcusd`).
